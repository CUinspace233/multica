...
--
-- Renumbered from 164 → 225 → 273 → 349 across the four weekly upstream syncs
-- (2026-07-12, 2026-07-26, 2026-08-10, 2026-08-18) because each sync upstream
-- landed its own 225_chat_message_channel_media_pending, 273_agent_task_queue_
-- runtime_id_index, etc. that occupied the fork-owned slot. The prefix
-- changes the schema_migrations key, so the runner re-applies this on
-- databases that already applied it as 164 / 225 / 273; ADD COLUMN IF NOT
-- EXISTS makes that idempotent (mirrors upstream's 164_attachment_task_id
-- IF NOT EXISTS pattern used for the same self-heal reason — see comment
-- in that file).
