...
-- Renumbered from 166 → 227 → 275 → 351 across the four weekly upstream syncs
-- (2026-07-12, 2026-07-26, 2026-08-10, 2026-08-18) because each sync upstream
-- landed its own 227_channel_media_pending_object, 275_task_token_agent_id_
-- index, etc. that occupied the fork-owned slot. The prefix changes the
-- schema_migrations key, so the runner re-applies this on databases that
-- already applied it as 166 / 227 / 275; CREATE TABLE IF NOT EXISTS makes
-- that idempotent.
